...
class OccEfficiencyHook(Hook):
    ...
    def construct_input(self, DUMMY_SHAPE=None, m_info=None):
        ...
        if 'lidar2ego_rotation' in m_info.keys():
            input['lidar2ego_rotation'] = torch.stack(m_info['lidar2ego_rotation'], dim=1)
            
        if 'lidar2ego_translation' in m_info.keys():
            input['lidar2ego_translation'] = torch.stack(m_info['lidar2ego_translation'], dim=1)
            
        if 'dataset_flag' in m_info.keys():
            input['dataset_flag'] = m_info['dataset_flag'].data[0]
