'''
Author: EASON XU
Date: 2024-02-01 05:28:37
LastEditors: EASON XU
Version: Do not edit
LastEditTime: 2024-02-07 13:03:23
Description: 头部注释
FilePath: /UniLiDAR/projects/unilidar_plugin/occupancy/dense_heads/occ_head_ori.py
'''
import copy
import numpy as np
import torch
...
class OccHead_ori(nn.Module):
    def __init__(
        ...
        super(OccHead_ori, self).__init__()
            ...
            if self.sample_from_img or self.sample_from_voxel:
                ...
                for b in range(B):
                    ...
                    this_coarse_coord = torch.stack([coarse_coord_x[coarse_occ_mask[b]],
                                                    ...
                                                    coarse_coord_z[coarse_occ_mask[b]]], dim=0).reshape(3, -1).contiguous()  # 3, N
                    if self.training:
                        this_fine_coord = coarse_to_fine_coordinates(this_coarse_coord, self.cascade_ratio, topk=self.fine_topk).reshape(3,-1).contiguous()  # 3, 8N/64N
                    else:
                        this_fine_coord = coarse_to_fine_coordinates(this_coarse_coord, self.cascade_ratio).reshape(3,-1).contiguous()  # 3, 8N/64N
