'''
Author: EASON XU
Date: 2024-01-16 07:27:14
LastEditors: EASON XU
Version: Do not edit
LastEditTime: 2024-02-21 03:09:40
Description: 头部注释
FilePath: /UniLiDAR/projects/unilidar_plugin/occupancy/norm/uni3d_norm_2_in.py
'''
...
class _UniNorm(Module):
    ...
    def forward(self, input):
        ...
        if self.training :  ## train mode
            ...
            # input_source = input[:batch_size]
            # input_target = input[batch_size:]
            input_source = input[::2]  # 从索引0开始，每隔一个元素提取，获取源数据
            input_target = input[1::2]  # 从索引1开始，每隔一个元素提取，获取目标数据
            ...
            if input.dim() == 2:
                ...
                alpha = alpha.view(1, self.num_features, 1, 1)
            # elif input.dim() == 5:
            #     alpha = alpha.view(1, self.num_features, 1, 1, 1)
            ## Attention
            return z * (1 + alpha.detach())
...
NORM_LAYERS.register_module('UniNorm', module=_UniNorm)
NORM_LAYERS.register_module('UniNorm1d', module=UniNorm1d)
NORM_LAYERS.register_module('UniNorm2d', module=UniNorm2d)          
NORM_LAYERS.register_module('UniNorm3d', module=UniNorm3d)