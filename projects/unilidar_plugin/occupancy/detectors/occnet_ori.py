'''
Author: EASON XU
Date: 2024-02-01 05:30:27
LastEditors: EASON XU
Version: Do not edit
LastEditTime: 2024-02-07 14:09:45
Description: 头部注释
FilePath: /UniLiDAR/projects/unilidar_plugin/occupancy/detectors/occnet_ori.py
'''
import torch
import collections 
...
class OccNet_ori(BEVDepth):
    ...
    def simple_test(self, img_metas, img=None, points=None, rescale=False, points_occ=None, 
        ...
        if output['output_voxels_fine'] is not None:
            if output['output_coords_fine'] is not None:
                ...
                fine_coord[0] = torch.clamp(fine_coord[0], 0, gt_occ.shape[1]-1)
                fine_coord[1] = torch.clamp(fine_coord[1], 0, gt_occ.shape[2]-1)
                fine_coord[2] = torch.clamp(fine_coord[2], 0, gt_occ.shape[3]-1)
    ...
    def evaluation_semantic(self, pred, gt, eval_type, visible_mask=None):
        ...
        # ignore noise
        noise_mask = gt != 0 if H==256 else 255
    ...
    def evaluation_semantic_sk(self, pred, gt, eval_type, visible_mask=None):
        _, H, W, D = gt.shape
        pred = F.interpolate(pred, size=[H, W, D], mode='trilinear', align_corners=False).contiguous()
        pred = torch.argmax(pred[0], dim=0).cpu().numpy()
        gt = gt[0].cpu().numpy()
